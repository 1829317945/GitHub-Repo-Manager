...
if ! gh auth status &> /dev/null; then
    echo -e "${RED}错误：gh 未登录，请先执行 gh auth login。${NC}"
    exit 1
fi
...
function rename_and_relink() {
    local new_url owner repo_to_rename original_url CURRENT_REMOTE
    ...
    if [ -d ".git" ] && git remote get-url origin &> /dev/null; then
        ...
        if [[ "$USE_CURRENT" == "y" || "$USE_CURRENT" == "Y" ]]; then  # ✅ 修复 $
    ...
    [ -z "$NEW_NAME" ] && echo "取消操作。" && return

    # ── 先改名 ──
    gh repo rename "$NEW_NAME" --repo "$repo_to_rename" --yes || return
    echo -e "${GREEN}仓库已成功改名为：$NEW_NAME${NC}"

    # ── 自动查找并更新所有关联了该仓库的本地目录 ──
    old_repo_name="${repo_to_rename##*/}"
    echo -e "${BLUE}正在搜索关联了 $old_repo_name 的本地目录...${NC}"

    while IFS= read -r git_config; do
        local_dir=$(dirname "$git_config")
        local_dir=$(dirname "$local_dir")
        if grep -q "$old_repo_name" "$git_config"; then
            new_url="git@github.com:$owner/$NEW_NAME.git"
            git -C "$local_dir" remote set-url origin "$new_url"
            echo -e "${GREEN}已更新：$local_dir → $new_url${NC}"
        fi
    done < <(find "$HOME" -name "config" -path "*/.git/config" 2>/dev/null)
}
